...
<html lang="en">
    <head>
        ...
        function calculateTotal() {
            var harga = document.getElementById('tanggal_pinjam').value;
            var lama = document.getElementById('kembali').value;
    ...
    <body class="sb-nav-fixed">
                ...
                <main>
                    <div class="container-fluid px-4">
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="row">
                            <div class="col-md-6 ">
                                <form method="POST" action="insert_peminjaman.php">
                                <div class="mb-3">
                            
                                    <label for="identitas_peminjam" class="form-label">Identitas Peminjam:</label>
                                    <input type="text" id="identitas_pinjam" name="identitas_peminjam" class="form-control" list="identitas" placeholder="Nama Peminjam">
                                    <datalist id="identitas">
                                        
                                        <?php
                                            include "../config/koneksi.php";
                                            $tampil=mysqli_query($koneksi,"SELECT * FROM anggota");
                                            while($data=mysqli_fetch_array($tampil)){echo"<option value=' [$data[nis]] $data[nama]'> </option>";}
                                        ?>
                                    </datalist>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="judul_buku" class="form-control" id="judul_buku" list="judul" placeholder="Masukkan Judul Buku" required>
                                    <datalist id="judul">
                        
                                        <?php
                                            include "../config/koneksi.php";
                                            $tampil=mysqli_query($koneksi,"SELECT * FROM buku");
                                            while($data=mysqli_fetch_array($tampil)){echo"<option value=' [$data[id]] $data[judul_buku]'> </option>";}
                                        ?>
                                    </datalist>
                                </div>
                            ...
                            <div class="col-md-6 ">
                                <div class="mb-3">
                                    ...
                                    <input name="tanggal_pinjam" type="date" class="form-control" id="tanggal_pinjam" value="<?php echo date('Y-m-d'); ?>">

                                </div>
                                <div class="mb-3">
                                    <label for="batas_pinjam" class="form-label">Batas Pinjam :</label>
                                <input type="date" class="form-control" name="batas_pinjam" id="batas_pinjam" readonly>
                                </div>
                                <div class="mb-3">
                                    <label for="jumlah" class="form-label">Jumlah :</label>
                                    <input type="number" name="jumlah" class="form-control" id="jumlah" placeholder="Masukkan Jumlah Buku" required>
                                </div>
                                
                                
                                <div class="mb-3">
                                    <a href="data_peminjaman.php" class="btn btn-warning">Kembali</a> <button type="submit" class="btn btn-primary">Simpan</button>
                                </div>
                            </form>
                            </div>
                            </div>
        ...
        const batasKembaliInput = document.getElementById("batas_pinjam");

        // set the minimum date of tanggal_kembali to be the day after tanggal_pinjam
        tanggalPinjamInput.addEventListener("change", () => {
            ...
            batasKembaliInput.min = tanggalPinjam.toISOString().split("T")[0];

            // set the value of tanggal_kembali to be the day after tanggal_pinjam
            tanggalKembaliInput.value = new Date(tanggalPinjam.setDate(tanggalPinjam.getDate() + 1)).toISOString().split("T")[0];
        });
        </script>
